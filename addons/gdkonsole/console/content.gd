...
func open_logfile():
    if !GDKonsole.config.enable_log_to_file:
        return; # Logfile disabled
    # Open and create if not existing
    if !FileAccess.file_exists(GDKonsole.config.logfile_path):
        logfile = FileAccess.open(GDKonsole.config.logfile_path, FileAccess.WRITE_READ);
    else:
        logfile = FileAccess.open(GDKonsole.config.logfile_path, FileAccess.READ_WRITE);
    ...
    if logfile != null:
        ...
        GDKonsole.config.enable_log_to_file = false;

func write(msg: String = "", color: Color = GDKonsole.colors.default):
    # Remove lines exceeding quota
    while get_line_count() > GDKonsole.config.max_lines:
        remove_paragraph(0);
